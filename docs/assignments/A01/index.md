# A1 – Assignment

## Analyze
## Task A:  Portfolio Analysis 
Portfolio analysis on NateKarau61. This portfolio is set up very well and can be navigated with ease allowing the reader to find what they need in seconds. The projects in the portfolio are all given with pictures and engineering drawings with dimensions so that anyone can replicate the work shown. This portfolio only shows the finished product of projects as a way of presentation and does not show any steps in the building or design process. The language used on this portfolio would meet the standards of a document that an employer would use.
Portfolio analysis on

## Task B:  Product Analysis

## Decide


## Communicate

